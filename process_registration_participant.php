...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt_presenter = $mysqli->prepare("INSERT INTO tbl_participants (
        ...
        if ($stmt_presenter) {
            $stmt_presenter->bind_param("sissssissssssssssssssss",
                $member_status, $is_affiliated_member, $title, $fullname, $education_degrees,
                $gender, $age, $shirt_size, $institution_name, $country, $department,
                $job_title, $airport_pickup,
                $arrival_date, $flight_number_arrival, $on_campus_checkin, $airport_dropoff,
            ...
            if (!$isMember && !empty($_FILES['registration_receipt']['name'])) {
                ...
                $stmt_receipt = $mysqli->prepare("INSERT INTO payment_receipts (presenter_id, receipt_type, file_path) VALUES (?, ?, ?)");
                if ($stmt_receipt) {
            ...
            if ($wants_tour && !empty($_FILES['tour_receipt']['name'])) {
                ...
                $stmt_tour_receipt = $mysqli->prepare("INSERT INTO payment_receipts (presenter_id, receipt_type, file_path) VALUES (?, ?, ?)");
                if ($stmt_tour_receipt) {
                    ...
                    $stmt_tour_update = $mysqli->prepare("UPDATE presenters SET tour_receipt_path = ? WHERE id = ?");
                    if ($stmt_tour_update) {
